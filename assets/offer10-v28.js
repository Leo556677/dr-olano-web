(()=>{
  ...
  function prices(o){const c=o?.claim||{};return c.regular_price_pen==null?{old:'Según evaluación médica',now:discountText()+' menos sobre valor evaluado'}:{old:money(c.regular_price_pen),now:money(c.discounted_price_pen)}}
    ...
    if(!$('#offer10Overlay'))document.body.insertAdjacentHTML('beforeend',`
      <div id="offer10Overlay" aria-hidden="true">
        <div id="offer10Dialog" role="dialog" aria-modal="true" aria-labelledby="offer10Title" tabindex="-1">
          ...
          <h2 class="offer10-title" id="offer10Title">ACTIVA TU <span class="offer10-title-accent">${discountText()}</span> DE DESCUENTO</h2>
          ...
          <div class="offer10-prices">
            ...
            <div class="offer10-price"><small>CON TU ${discountText()} APLICADO</small><div class="offer10-new" id="offer10New"></div></div>
          </div>
  ...
  function renderOfferInBanner(){
    ...
    line.innerHTML=`<b class="offer10-inline-badge">${o.redeemed?'✓ '+discountText()+' APLICADO':discountText()+' OFERTA'}</b>${fixed?`<em class="offer10-inline-price">${esc(p.old)} → ${esc(p.now)}</em>`:`<em class="offer10-inline-price">${discountText()} sobre valor evaluado</em>`}<time class="offer10-inline-time">${showExpired?'VENCIDO':timeText(o)}</time>`;
  ...
  Promise.resolve(window.olanoBusinessConfigReady).catch(()=>null).finally(init);
})();
